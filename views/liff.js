...
function renderMemberCard(data){const rewardReady=data.currentStamps>0&&data.currentStamps%10===0;const filledCount=rewardReady?10:(data.currentStamps%10);const remaining=10-filledCount;let circlesHtml='';for(let i=0;i<10;i++){const filled=i<filledCount;circlesHtml+='<div style="width:28px;height:28px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;margin:3px;border:2px solid '+(filled?'#ff8f00':'#e0e0e0')+';'+(filled?'background:linear-gradient(135deg,#ff6f00 0%,#ff8f00 100%);color:#fff;':'background:#f5f0e8;color:#bdbdbd;')+'font-size:.9rem;font-weight:bold">'+(filled?'☕':'')+'</div>'}const trackerHtml='<div style="display:flex;flex-wrap:wrap;justify-content:center">'+circlesHtml+'</div>';const messageHtml=rewardReady?'<p style="text-align:center;font-size:1.1rem;font-weight:bold;color:#388e3c;margin-top:0.5rem">🎉 Reward Ready!</p>':'<p style="text-align:center;font-size:0.9rem;color:#8d6e63;margin-top:0.5rem">'+remaining+' more stamp(s) until your next reward</p>';return '<div class="card"><div class="card-header">'+(data.pictureUrl?'<img src="'+data.pictureUrl+'" alt="'+data.displayName+'">':'')+'<h2>'+data.displayName+'</h2></div><div class="card-body"><div class="stamp-progress" style="margin-bottom:1.5rem"><div style="text-align:center;margin-bottom:0.5rem"><span style="font-size:2rem;font-weight:bold;color:#5b3a29">'+data.currentStamps+'</span><span style="font-size:1.2rem;color:#8d6e63"> / 10 stamps</span></div>'+trackerHtml+messageHtml+'</div><div class="stats"><div class="stat"><div class="stat-value">'+data.points+'</div><div class="stat-label">Points</div></div><div class="stat"><div class="stat-value">'+data.availableRewards+'</div><div class="stat-label">Rewards Available</div></div></div><p style="font-size:.85rem;color:#8d6e63;margin:1rem 0">Member since: '+formatDate(data.memberSince)+'<br>Member UID: '+data.memberUid+'</p><div class="qr-container"><h3>Your Member QR</h3><img src="'+data.qrCode+'" alt="Member QR Code"></div></div></div>'}
