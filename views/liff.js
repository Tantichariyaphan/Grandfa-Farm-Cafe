...
.stamp-dots{ display:flex; justify-content:center; gap:10px; flex-wrap:wrap; margin-top:12px; } .stamp-dot{ width:38px; height:38px; border-radius:50%; border:2px solid #d7b899; background:white; display:flex; align-items:center; justify-content:center; font-weight:bold; } .stamp-dot.filled{ background:#6d4c41; color:white; border-color:#6d4c41; }
...
function renderMemberCard(data){const currentCycle = data.currentStamps % 10;const progress = (currentCycle / 10) * 100 ;const remaining = currentCycle === 0 ? 10 : 10 - currentCycle;return '<div class="card"><div class="card-header">'+(data.pictureUrl?'<img src="'+data.pictureUrl+'" alt="'+data.displayName+'">':'')+'<h2>'+data.displayName+'</h2></div><div class="card-body">'<div class="stamp-dots">' +Array.from({length:10},(_,i)=> '<div class="stamp-dot '+(i<currentCycle?'filled':'')+'">'+(i+1)+'</div>' ).join('') +'</div>'<div class="stamp-bar"><div class="stamp-fill" style="width:'+progress+'%">'+data.currentStamps+'/10</div></div><p style="text-align:center;font-size:0.9rem;color:#8d6e63;margin-top:0.5rem">'+remaining+' more stamp(s) until your next reward</p></div><div class="stats"><div class="stat"><div class="stat-value">'+data.points+'</div><div class="stat-label">Points</div></div><div class="stat"><div class="stat-value">'+Math.floor(data.currentStamps/10)+'</div><div class="stat-label">Rewards Available</div></div></div><p style="font-size:.85rem;color:#8d6e63;margin:1rem 0">Member since: '+formatDate(data.memberSince)+'<br>Member UID: '+data.memberUid+'</p><div class="qr-container"><h3>Your Member QR</h3><img src="'+data.qrCode+'" alt="Member QR Code"></div></div></div>'}
