...
function renderMemberCard(data){const progress=(data.currentStamps/10)*100;const remaining=10-(data.currentStamps%10);return '<div class="card"><div class="card-header">'+(data.pictureUrl?'<img src="'+data.pictureUrl+'" alt="'+data.displayName+'">':'')+'<h2>'+data.displayName+'</h2></div><div class="card-body"><div class="stamp-progress" style="margin-bottom:1.5rem"><div style="text-align:center;margin-bottom:0.5rem"><span style="font-size:2rem;font-weight:bold;color:#5b3a29">'+data.currentStamps+'</span><span style="font-size:1.2rem;color:#8d6e63"> / 10 stamps</span></div><div class="stamp-bar"><div class="stamp-fill" style="width:'+progress+'%">'+data.currentStamps+'/10</div></div><p style="text-align:center;font-size:0.9rem;color:#8d6e63;margin-top:0.5rem">'+remaining+' more stamp(s) until your next reward</p></div><div class="stats"><div class="stat"><div class="stat-value">'+data.points+'</div><div class="stat-label">Points</div></div><div class="stat"><div class="stat-value">'+Math.floor(data.currentStamps/10)+'</div><div class="stat-label">Rewards Available</div></div></div><p style="font-size:.85rem;color:#8d6e63;margin:1rem 0">Member since: '+formatDate(data.memberSince)+'<br>Member UID: '+data.memberUid+'</p><div class="qr-container"><h3>Your Member QR</h3><img src="'+data.qrCode+'" alt="Member QR Code"></div></div></div>'}
