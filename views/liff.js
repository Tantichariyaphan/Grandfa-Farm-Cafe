...
.card-header{     background-size:cover !important;     background-position:center !important;     background-repeat:no-repeat !important;      min-height:260px;      display:flex;     flex-direction:column;     align-items:center;     justify-content:center;      padding:24px;     color:white;     text-align:center;      position:relative; }
.card-header img{width:80px;height:80px;border-radius:50%;border:3px solid #fff;margin-bottom:.5rem; }
.card-body{padding:1.5rem}
.member-tier{margin-top:8px;font-size:1rem;font-weight:700;color:#fff;}
.member-tagline{margin-top:4px;font-size:.8rem;color:rgba(255,255,255,.9);line-height:1.4;}
...
.stamp-icon{width:38px;height:38px;object-fit:contain;}
...
function renderMemberCard(data) { const currentCycle = data.currentStamps % 10; const remaining = currentCycle === 0 ? 10 : 10 - currentCycle; return '' + '<div class="card member-card">      <div class="card-header"          style="background:url(' + data.memberCardBackground + ') center/cover no-repeat;">' + (data.pictureUrl ? '<img src="' + data.pictureUrl + '" alt="' + data.displayName + '">' : '') + '<h2>' + data.displayName + '</h2>' + '<div class="member-tier">' + data.membershipName + '</div>' + '<div class="member-tagline">' + data.membershipTagline + '</div>' + '</div>' + '<div class="card-body">' + '<div class="stamp-progress" style="margin-bottom:1.5rem">' + '<div style="text-align:center;margin-bottom:1rem">' + '<span style="font-size:2rem;font-weight:bold;color:#5b3a29">' + data.currentStamps + '</span>' + '<span style="font-size:1.2rem;color:#8d6e63"> / 10 stamps</span>' + '</div>' + '<div class="stamp-dots">' + Array.from({ length: 10 }, (_, i) => '<div class="stamp-dot ' + (i < currentCycle ? 'filled' : '') + '">' + ( i < currentCycle ? '<img src="/pic/membercard/stamp.webp" alt="Stamp" class="stamp-icon">' : '' ) + '</div>' ).join('') + '</div>' + '<p style="text-align:center;font-size:0.9rem;color:#8d6e63;margin-top:1rem">' + remaining + ' more stamp(s) until your next reward' + '</p>' + '</div>' + '<div class="stats">' + '<div class="stat">' + '<div class="stat-value">' + data.points + '</div>' + '<div class="stat-label">Points</div>' + '</div>' + '<div class="stat">' + '<div class="stat-value">' + data.availableRewards + '</div>' + '<div class="stat-label">Rewards Available</div>' + '</div>' + '</div>' + '<p style="font-size:.85rem;color:#8d6e63;margin:1rem 0">' + 'Member since: ' + formatDate(data.memberSince) + '</p>' + '<div class="qr-container">' + '<h3>Your Member QR</h3>' + '<img src="' + data.qrCode + '" alt="Member QR Code">' + '</div>' + '</div>' + '</div>'; }
function renderProfile(data) { return '' + '<form id="profileForm" class="card">' + '<div class="card-header">' + (data.pictureUrl ? '<img src="' + data.pictureUrl + '" alt="' + data.displayName + '">' : '') + '<h2>' + data.displayName + '</h2>' + '</div>' + '<div class="card-body">' + '<div class="form-group">' + '<label>Phone Number</label>' + '<input id="phone" name="phone" value="' + (data.phone || '') + '">' + '</div>' + '<div class="form-group">' + '<label>Birthday</label>' + '<input id="birthday" name="birthday" type="date" value="' + (data.birthday ? data.birthday.substring(0,10) : '') + '">' + '</div>' + '<div style="display:flex;gap:.5rem">' + '<button id="saveProfileBtn" type="submit">Save Changes</button>' + '<button id="cancelProfileBtn" type="button" class="secondary">Cancel</button>' + '</div>' + '<div id="profileMessage"></div>' + '</div>' + '</form>'; }
...
const renderers = { card: renderMemberCard, coupons: renderCoupons, stamps: renderStampHistory, rewards: renderRewardHistory };
...
async function showCouponQR(code){try{clearQrTimer();const data=await api('/api/member/coupons/'+encodeURIComponent(code)+'/qr');currentSessionId=data.sessionId||data.sessionId;document.getElementById('modalTitle').textContent=(data.coupon && data.coupon.title) || data.title || 'Coupon';renderQrSession(data);document.getElementById('modal').classList.remove('hidden');currentCoupon = data.coupon || { code: code, title: data.title, description: data.description, expires_at: data.expires_at, status: data.status };}catch(e){alert('Failed to generate QR: '+e.message)}}
...
async function setupProfileForm(){
  ...
  form.onsubmit=async e=>{
    ...
    try{
      ...
      setTimeout(()=>view('profile'),1500);
    }catch(e){msg.className='error';msg.textContent='⚠️ '+e.message}
    finally{btn.disabled=false;btn.textContent='Save Changes'}
  }
  if(cancel){cancel.onclick=()=>{view('card')}}
}
