...
async function scanMemberQR(){try{if(typeof liff==='undefined'||!liff.scanCode){throw Error('Camera scanning not available')}const result=await liff.scanCode();if(!result||!result.value){throw Error('No QR code detected')}await processMemberToken(result.value)}catch(e){if(e.message.includes('not available')){showManualStampInput();showResultBanner('Camera not available. Please paste QR token manually.','error')}else if(e.message.includes('denied')||e.message.includes('permission')){showResultBanner('Camera permission denied. Please allow camera access or use manual input.','error');showManualStampInput()}else{showResultBanner('Failed to scan: '+e.message,'error')}}}

async function scanCouponQR(){try{if(typeof liff==='undefined'||!liff.scanCode){throw Error('Camera scanning not available')}const result=await liff.scanCode();if(!result||!result.value){throw Error('No QR code detected')}await processCouponToken(result.value)}catch(e){if(e.message.includes('not available')){showManualCouponInput();showResultBanner('Camera not available. Please paste QR token manually.','error')}else if(e.message.includes('denied')||e.message.includes('permission')){showResultBanner('Camera permission denied. Please allow camera access or use manual input.','error');showManualCouponInput()}else{showResultBanner('Failed to scan: '+e.message,'error')}}}
...
async function confirmStamp(){const btn=document.getElementById('confirmStampBtn');try{btn.disabled=true;btn.textContent='Adding Stamp…';const scan=await api('/api/stamp/scan',{method:'POST',body:JSON.stringify({qrToken:document.getElementById('memberToken').value.trim()})});const result=await api('/api/stamp/add',{method:'POST',body:JSON.stringify({memberUid:scan.member_uid,drinkQuantity:selectedQuantity,idempotencyKey:idempotencyKey})});showResultBanner('✓ Added '+result.stamps_earned+' stamp(s). Member now has '+result.new_stamp_count+' stamps.','success');if(result.newCoupons&&result.newCoupons.length>0){const couponNames=result.newCoupons.map(c=>c.title).join(', ');setTimeout(()=>showResultBanner('🎉 Member earned a reward coupon: '+couponNames,'success'),1500)}document.getElementById('memberToken').value='';document.getElementById('memberPreview').classList.add('hidden');selectedQuantity=1;updateQuantityDisplay();btn.disabled=false;btn.textContent='✓ Confirm & Add Stamp';idempotencyKey=null}catch(e){btn.disabled=false;btn.textContent='✓ Confirm & Add Stamp';showResultBanner(e.message,'error')}}
...
function renderProfile(data){
  const birthdayValue=data.birthday?new Date(data.birthday).toISOString().split('T')[0]:'';
